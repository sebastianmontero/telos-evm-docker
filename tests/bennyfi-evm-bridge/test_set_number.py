...
def test_all(tevmc_local):
    ...
    benybridge_account = 'benybridge'
    tevmc.cleos.deploy_contract_from_path(
            benybridge_account,
            Path('../bennyfi-evm-bridge/zero/build/benybridge'),
            contract_name=benybridge_account
        )
    tevmc.cleos.create_evm_account(benybridge_account, random_string())
    eth_addr = tevmc.cleos.eth_account_from_name(benybridge_account)
    assert eth_addr
    quantity_native = Asset.from_str('1000.0000 TLOS')
    quantity_evm = Asset.from_str('100.0000 TLOS')
    tevmc.cleos.transfer_token('eosio', benybridge_account, quantity_native, 'evm test')
    tevmc.cleos.transfer_token(benybridge_account, 'eosio.evm', quantity_evm, 'Deposit')
    assert local_w3.eth.get_transaction_count(local_w3.to_checksum_address(eth_addr)) == 1
    
    dummy_contract = tevmc.cleos.eth_deploy_contract_from_json(
            Path('../bennyfi-evm-bridge/evm/artifacts/contracts/DummyContract.sol/DummyContract.json'),
            'dummyContract'
        )
    
    assert dummy_contract.functions.number().call() == 0

    tevmc.cleos.logger.info('Calling init action... %s', dummy_contract.address[2:]) 
    tevmc.cleos.push_action(
        benybridge_account,
        'init',
        [dummy_contract.address[2:],dummy_contract.address[2:],benybridge_account, 'v1.0', benybridge_account],
        benybridge_account
    )

    tevmc.cleos.logger.info('Setting number...') 
    tevmc.cleos.push_action(
        benybridge_account,
        'setnumber',
        [11],
    ...
    assert dummy_contract.functions.number().call() == 11
